...
const long=location.longitude;
  ...
  return (
    <div className='flex items-center justify-between text-sm py-0 mb-5 border-b border-b-gray-400 navbar-light bg-light'>
      <img onClick={()=>navigate('/')} className='cursor-pointer' src={docpro} alt="" style={{height:120,width:140,borderRadius:20,padding:10}} />
      {/* <ul className='hidden md:flex items-start gap-10 font-medium'>
        ...
        <NavLink to='/allLawyers'>
            <li className='py-1'>ALL LAWYERS</li>
            <hr className='border-none outline-none h-0.5 bg-purple-400 w-3/5 m-auto hidden'/>
        </NavLink>
        <NavLink to='/Lawyerschedule'>
            <li className='py-1'>SCHEDULE</li>
            <hr className='border-none outline-none h-0.5 bg-purple-400 w-3/5 m-auto hidden'/>
        </NavLink>
        <NavLink to='/LawyerAbout'>
            <li className='py-1'>ABOUT</li>
            <hr className='border-none outline-none h-0.5 bg-purple-400 w-3/5 m-auto hidden'/>
        </NavLink>
        <NavLink to='/LawyerContact'>
      ...
      {error && <p style={{ color: "red" }}>{error}</p>}
                ...
                <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
                    <div className='min-w-48 bg-green-50 rounded flex flex-col gap-4 p-4'>
                        ...
                        {/* <p onClick={()=> navigate('my-appointments')} className='hover:text-black cursor-pointer'>Settings</p> */}
        ...
        <div className={` ${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
             { LawyerNav.map((item) => ( <NavLink  onClick={()=>setShowMenu(false)} to={item.to}><p className='px-4 py-2 rounded inline-block'>{item.name}</p></NavLink>))}
            {/* <NavLink  onClick={()=>setShowMenu(false)} to='/'><p className='px-4 py-2 rounded inline-block'>Home</p></NavLink>
            <NavLink  onClick={()=>setShowMenu(false)} to='/allLawyers'><p className='px-4 py-2 rounded inline-block'>All Lawyers</p></NavLink>
            <NavLink  onClick={()=>setShowMenu(false)} to='/Lawyerschedule'><p className='px-4 py-2 rounded inline-block'>Schedules</p></NavLink>
            <NavLink  onClick={()=>setShowMenu(false)} to='/LawyerAbout'><p className='px-4 py-2 rounded inline-block'>About</p></NavLink>
            <NavLink  onClick={()=>setShowMenu(false)} to='/LawyerContact'><p className='px-4 py-2 rounded inline-block'>Contact</p></NavLink> */}
          </ul>
        </div>
      </div>
    </div>
